...
export class Engine {
    ...
    handleMouseOverEvents(clickable, event) {
        ...
        switch (actionTargetId.action) {
            case "PROJECT_CLICKS":
                ...
                this.displayManager.projectsContentManager.displayFeatureContent(`#${actionTargetId.targetId}`);
                break;
        }
        return;
    }
    handleMouseLeaveEvents(clickable, event) {
        console.log("handling mouse leave events");
    }
    handleClickEvents(clickable, event) {
        const actionTargetId = this.extractActionAndTargetFromDelegatedClick(event);
        ...
        switch (actionTargetId.action) {
            ...
            case "IGNORE":
                return;
            default:
                console.error(`unhandled action for delegated interaction: ${actionTargetId.action}`);
                break;
        }
    }
    ...
    handleReturnToSpaceship(clickable, event) {
        ...
        if (this.spaceSceneManager.checkIfActiveClickHasSceneMapping(state.activeClick) === true) {
            this.spaceSceneManager.returnToSpaceship(state.activeClick);
            this.monitorManager.reInitialize();
        }
        else {
            const lastValidElement = this.spaceSceneManager.findLastValidSceneElement(state.clickList);
